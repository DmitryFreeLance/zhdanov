...
- при превышении порогов отправляет тревогу с напоминанием или кнопкой запуска звонка
...
## Важное обновление MAX API

По официальной документации MAX, до `19 июля 2026` нужно:

- перевести запросы с `https://platform-api.max.ru` на `https://platform-api2.max.ru`
- добавить сертификат Минцифры в список доверенных

В этой версии проекта уже:

- дефолтный `APP_MAX_BASE_URL` переключён на `https://platform-api2.max.ru`
- Docker-контейнер умеет импортировать сертификаты из `/app/certs` в Java truststore при старте
- официальные сертификаты `Russian Trusted Root CA` и `Russian Trusted Sub CA` уже встроены в образ по умолчанию

Актуальные сертификаты Минцифры можно взять на [Госуслугах](https://www.gosuslugi.ru/crt).
...
- `👤 Аккаунты` — подключить WB-аккаунт по номеру и готовому файлу браузерной сессии, сделать аккаунт активным, поставить его на паузу или отключить
...
- `☎️ Включить/выключить дозвон` — управлять кнопкой запуска звонка для конкретного чата
...
4. Если строка превысила порог конкретного чата:
   ...
   - при включённом дозвоне показывает кнопку `Начать звонок`
   - звонок запускается только после нажатия этой кнопки
...
- Для входа в `WB` рекомендуется локальный browser bootstrap: пользователь логинится в реальном браузере, получает `storage state` файл, после чего бот подхватывает его по номеру телефона.
...
5. Откройте `👤 Аккаунты` → `📎 Подключить по номеру`, отправьте номер телефона. Если на сервере уже лежит файл сессии для этого номера, бот сразу подключит аккаунт.

## Локальная авторизация WB через ваш браузер

Самый надёжный сценарий сейчас такой:

1. На вашем Mac запустите:

```bash
chmod +x scripts/bootstrap_wb_session.sh
./scripts/bootstrap_wb_session.sh +79991234567
```

2. Откроется реальный браузер `Chromium`.
3. Войдите в `WB Logistics` под нужным номером.
4. После успешного входа файл сессии сохранится в:

```text
data/wb-session-imports/79991234567.json
```

5. Перенесите этот файл на сервер в ту же папку `data/wb-session-imports/`.
6. В боте откройте `👤 Аккаунты`, нажмите `📎 Подключить по номеру` и отправьте тот же номер. Бот найдёт файл и подключит аккаунт.
...
И положите сертификаты Минцифры в папку:

```bash
mkdir -p certs
```

Контейнер сам импортирует их из `/app/certs` в Java truststore при старте.
...
- `бот не отвечает в MAX`
  Для `Webhook` обычно не настроен публичный `HTTPS`, неверный webhook URL или не добавлен сертификат Минцифры. Для `Long Polling` обычно проблема в токене или в том, что пользователь не нажал `Начать`.
